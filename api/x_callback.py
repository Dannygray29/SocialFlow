...
async def callback(request: Request):
    ...
    fernet = _fernet()
    if not client_id or not client_secret or not fernet:
    ...
    token = response.json()
    encrypted = fernet.encrypt(json.dumps(token).encode()).decode()
    result = RedirectResponse("/?x_connected=true", status_code=302)
    result.set_cookie(
        "socialflow_x_token", encrypted, httponly=True, secure=True,
